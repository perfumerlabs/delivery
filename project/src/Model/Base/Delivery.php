...
/**
 * Base class that represents a row from the 'delivery_delivery' table.
 *
 *
 *
...
abstract class Delivery implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Delivery\\Model\\Map\\DeliveryTableMap';
    ...
    /**
     * @var        ChildNotification one-to-one related ChildNotification object
     */
    protected $singleNotification;
    ...
    /**
     * Initializes internal state of Delivery\Model\Base\Delivery object.
     * @see applyDefaults()
     */
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[DeliveryTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[DeliveryTableMap::COL_NAME] = true;
        }
    ...
    public function setStatus($v)
    {
        ...
        if ($this->status !== $v) {
            $this->status = $v;
            $this->modifiedColumns[DeliveryTableMap::COL_STATUS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : DeliveryTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->singleNotification !== null) {
                if (!$this->singleNotification->isDeleted() && ($this->singleNotification->isNew() || $this->singleNotification->isModified())) {
                    $affectedRows += $this->singleNotification->save($con);
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('delivery_delivery_id_seq')");
        ...
        if ($this->isColumnModified(DeliveryTableMap::COL_ID)) {
        ...
        $sql = sprintf(
            'INSERT INTO delivery_delivery (%s) VALUES (%s)',
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->singleNotification) {
                ...
                $result[$key] = $this->singleNotification->toArray($keyType, $includeLazyLoadColumns, $alreadyDumpedObjects, true);
            }
        }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Delivery\Model\Delivery
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Delivery\Model\Delivery The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(DeliveryTableMap::COL_ID)) {
            $criteria->add(DeliveryTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(DeliveryTableMap::COL_NAME)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Delivery\Model\Delivery (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            $relObj = $this->getNotification();
            if ($relObj) {
                $copyObj->setNotification($relObj->copy($deepCopy));
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Delivery\Model\Delivery Clone of current object.
     * @throws PropelException
     */
    ...
    /**
     * Gets a single ChildNotification object, which is related to this object by a one-to-one relationship.
     *
    ...
    public function getNotification(ConnectionInterface $con = null)
    {
        ...
        if ($this->singleNotification === null && !$this->isNew()) {
            $this->singleNotification = ChildNotificationQuery::create()->findPk($this->getPrimaryKey(), $con);
        }
    ...
    /**
     * Sets a single ChildNotification object as related to this object by a one-to-one relationship.
     *
     * @param  ChildNotification $v ChildNotification
     * @return $this|\Delivery\Model\Delivery The current object (for fluent API support)
     * @throws PropelException
     */
    public function setNotification(ChildNotification $v = null)
    {
        $this->singleNotification = $v;

        // Make sure that that the passed-in ChildNotification isn't already associated with this object
        if ($v !== null && $v->getDelivery(null, false) === null) {
            $v->setDelivery($this);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            if ($this->singleNotification) {
                $this->singleNotification->clearAllReferences($deep);
            }
        } // if ($deep)
